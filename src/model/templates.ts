...
export interface StarterTask {
	...
	depth: number;
	text: string;
	/** The character to put inside the brackets. " " unless the line asked for another. */
	status: string;
}

/** `- [x] Text`, `[x] Text`, `- Text`, `Text` — marker and checkbox both optional. */
const STARTER_PREFIX = /^(?:[-*+]\s+)?(?:\[(.)\]\s*)?/;
 ...
 *
 * A line may carry its own `[x]`-style checkbox, which is how the shipped template can
 * open with one subtask already done. The syntax is optional and the same one the board
 * itself uses, so a user editing this setting writes what they already read every day.
 *
...
export function parseStarterTasks(spec: string): StarterTask[] {
	...
	for (const raw of spec.split("\n")) {
		...
		const m = STARTER_PREFIX.exec(trimmed);
		const status = m?.[1] ?? " ";
		const text = trimmed.slice(m?.[0].length ?? 0).trim();
		if (text.length === 0) continue; // a line that was ONLY a checkbox has nothing to say
		...
		out.push({ depth: Math.max(0, Math.min(wanted, max)), text, status });
	}
	return out;
}
...
export function renderStarterTasks(spec: string, indentUnit: string): string[] {
	return parseStarterTasks(spec).map((t) => `${indentUnit.repeat(t.depth)}- [${t.status}] ${t.text}`);
}
